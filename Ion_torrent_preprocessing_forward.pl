...
=cut
	...
	my $forward_primer="GTGTGCCAGC.GCCGCGGTAA";
	my $reverse_primer="CCGGACTAC..GGGT.TCTAAT";
	...
	while (<FASTA>)
	{
		...
		if ($seq=~/$forward_barcode$forward_primer(.*)$reverse_primer$reverse_barcode/)
		{
			...
			$seq_num{$line[0]}++;
			$target=$1;
		}elsif ($seq=~/$forward_barcode$forward_primer(.*)$/)
		{
			$seq_num{"Total"}++;                                                                            
                        $seq_num{$line[0]}++;
                        $target=$1; 
		}
		print OUT ">",$line[0],"_",$n,"\n$target\n" if $target ne "" and length($target) >= 100;
		$n++ if $target ne "";
	}
